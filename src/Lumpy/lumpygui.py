...
from tkFont import * #@UnusedWildImport
...
class Gui(Tk):
    ...
    def grid(self, widget, i=None, j=None, **options):
        ...
        try:
            incr = options['columnspan'] #@UnusedVariable
        except KeyError:
            incr = 1 #@UnusedVariable
...
class Point(list):
    ...
    def setx(pos, val): pos[0] = val #@NoSelf
    def sety(pos, val): pos[1] = val #@NoSelf
...
def widget_demo():
    ...
    # a label
    la1 = g.la(text='This is a label.') #@UnusedVariable
    ...
    # a button
    bu = g.bu(side=TOP, text='Press me', command=press_me) #@UnusedVariable
    ...
    g.col()
    la = g.la(text='List of colors:') #@UnusedVariable
    ...
    # button
    bu = g.bu(text='Apply color', command=apply_color) #@UnusedVariable
    ...
    item4 = ca.create_image(200, 300, image=photo) #@UnusedVariable
    ...
    for size in [10, 12, 14, 15, 17, 20]:
        rb = g.rb(text=str(size), variable=fontsize, value=size, #@UnusedVariable
                  command=set_font)
