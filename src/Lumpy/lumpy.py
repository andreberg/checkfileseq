...
You can pass arbitrary Python statements on the command line (with the
positional source argument) and see the relationship graphed as a UML
diagram.

Example: 
    
    from checkfileseq import *; FileSequenceChecker(start=10)

This will create a new FileSequenceChecker object and draw a 
diagram for its internal state. Note that checkfileseq.py must
be present in the PYTHONPATH. You can also pass paths to be appended
to the PYTHONPATH with the -p/--paths=<string_of_colon_delimited_paths>
command line switch.
...
from lumpygui import * #@UnusedWildImport
...
class ClassDiagramClass(Thing):
    ...
    def drawme(self, diag, pos, flip, tags=tuple()):
        ...
        item = self.canvas.str(p, name, tags=tags) #@UnusedVariable
        p.y += 0.8
        ...
        for f in self.methods:
            item = self.canvas.str(p, f.__name__, tags=tags) #@UnusedVariable
            p.y += 1
        ...
        for varname in cvars:
            item = self.canvas.str(p, varname, tags=tags) #@UnusedVariable
            p.y += 1
...
class Lumpy(Gui):
    ...
    def opaque_module(self, modobj):
        ...
        for var, val in modobj.__dict__.iteritems(): #@UnusedVariable
...
class ClassDiagram(Diagram):
    ...
    def draw(self):
        ...
        if self.classes == None:
            drawn = self.draw_classes(roots, pos) #@UnusedVariable
        else:
            drawn = self.draw_classes(classes, pos) #@UnusedVariable
...
PROGRAM_BUILD_DATE = "2010-11-01"
PROGRAM_VERSION_MESSAGE = '%%(prog)s %s (%s)' % (PROGRAM_VERSION, PROGRAM_BUILD_DATE)
PROGRAM_SHORTDESC = '''lumpy -- UML diagrams for arbitrary Python code'''
PROGRAM_LICENSE = u'''%s

Copyright 2010-11-01 André Berg (Berg Media)
Licensed under the MIT License\nwww.opensource.org/licenses/mit-PROGRAM_LICENSE.php
''' % PROGRAM_SHORTDESC


class LumpyError(Exception):
    ''' Generic exception class to raise and log different fatal errors. '''
    def __init__(self, msg):
        super(LumpyError).__init__(type(self))
...
def main(argv=None): # IGNORE:C0111
    ...
    try:
        ...
        parser.add_argument('-o', '--objdiag', dest="objdiag", help="draw object diagram [default: %(default)s]", action="store_true")
        parser.add_argument('-c', '--classdiag', dest="classdiag", help="draw class diagram [default: %(default)s]", action="store_true")
        parser.add_argument('-p', '--paths', dest="paths", help="paths to add to PYTHONPATH. Use normal colon separated path strings. [default: %(default)s]", metavar="PATH")
        parser.add_argument(dest="source", help=r"python statements to execute when building the UML graph. Define multiple statements in one string by separating them by a linefeed (\n), comma (,) or semicolon (;). Important: must end with a statement that defines a variable ___lumpyobj___ by assigning the object to draw. E.g.: ___lumpyobj___ = <objname>()")

        parser.set_defaults(objdiag=True, classdiag=False)
        # Process options
        args = parser.parse_args()
        
        paths = args.paths
        source = args.source
        objdiag = args.objdiag
        classdiag = args.classdiag
        ...
        if not re.match(r'.*?[a-zA-Z0-9_-]+\(.*?\).*?$', source):
            raise LumpyError("source statement doesn't end with '<objectname>(<args>)'!")
        
        # source given from command line
        lumpy = Lumpy()
        lumpy.make_stack()
        statements = re.split(r'[\n;,]', source)
        i = 1
        nstatements = len(statements)
        for statement in statements:
            if i >= nstatements:
                exec "___xobjx___ = %s" % statement
                if not isinstance(type(___xobjx___), (str, object, type(main))): #@UndefinedVariable
                    raise LumpyError("___xobjx___ not of type string, object or function!")
            else:
                exec statement.strip()
            i += 1
        if objdiag:
            if verbose > 0:
                print "Displaying object diagram..."
            lumpy.object_diagram(___xobjx___) #@UndefinedVariable
        if classdiag:
            if verbose > 0:
                print "Displaying class diagram..."
            lumpy.class_diagram()
        return 0
...
if __name__ == "__main__":
    if DEBUG:
        #sys.argv.append("-h")
        #sys.argv.append("-v")
        sys.argv.append("--paths=/Users/andre/Documents/Aptana Studio Workspace 2.0/Python Command Line Tools/src/File Sequence Checker")
        sys.argv.append("-c")
        sys.argv.append("from checkfileseq import *; FileSequenceChecker(start=10)")
    if TESTRUN:
        pass
    if UNITTESTS:
        pass
